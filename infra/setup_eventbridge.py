...
def setup_auto_deploy_trigger():
    ...
    try:
        role = iam_client.create_role(
            RoleName="LambdaAdultDeployTriggerRole",
        ...
        role_arn = f"arn:aws:iam::{account}:role/LambdaAdultDeployTriggerRole"
...
def lambda_handler(event, context):
    ...
    # Token stored as PLAINTEXT (not JSON)
    token = secret["SecretString"]

    repo = "AswiniOmsakthi/adult-income-sagemaker"
...
'''
    ...
    try:
        lambda_client.create_function(
            ...
            Description="Trigger GitHub Actions Pipeline 3 on Adult Income model approval",
            Timeout=30
        )
    ...
    print(f"   Watches    : AdultIncomeGroup")
